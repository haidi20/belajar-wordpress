	...
	switch ( $eggnews_widgets_field_type ) {
		...
		case 'widget_layout_image':
			...
			<div class="layout-image-wrapper">
				...
				<img src="<?php echo esc_url( $eggnews_widgets_layout_img ); ?>"
				     title="<?php echo esc_attr__( 'Widget Layout', 'eggnews-pro' ); ?>" alt=""/>
			</div>
			<?php
			break;
		...
		case 'selector':
			...
			<p><span class="field-label"><label
						class="field-title"><?php echo esc_html( $eggnews_widgets_title ); ?></label></span></p>
			<?php
		...
		case 'upload' :
			...
			$output .= '<input id="' . $id . '" class="upload' . $class . '" type="text" name="' . $name . '" value="' . $value . '" placeholder="' . __( 'No file chosen', 'eggnews-pro' ) . '" />' . "\n";
			if ( function_exists( 'wp_enqueue_media' ) ) {
				if ( ( $value == '' ) ) {
					$output .= '<input id="upload-' . $id . '" class="wid-upload-button button" type="button" value="' . __( 'Upload', 'eggnews-pro' ) . '" />' . "\n";
				} else {
					$output .= '<input id="remove-' . $id . '" class="wid-remove-file button" type="button" value="' . __( 'Remove', 'eggnews-pro' ) . '" />' . "\n";
				}
			} else {
				$output .= '<p><i>' . __( 'Upgrade your version of WordPress for full media support.', 'eggnews-pro' ) . '</i></p>';
			}

			$output .= '<div class="screenshot upload-thumb" id="' . $id . '-image">' . "\n";

			if ( $value != '' ) {
				$remove = '<a class="remove-image">' . __( 'Remove', 'eggnews-pro' ) . '</a>';
				$image  = preg_match( '/(^.*\.jpg|jpeg|png|gif|ico*)/i', $value );
				if ( $image ) {
					$output .= '<img src="' . esc_url( $value ) . '" alt="' . esc_attr__( 'Upload image', 'eggnews-pro' ) . '" alt=""/>';
				} else {
					$parts = explode( "/", $value );
					...
					$title = __( 'View File', 'eggnews-pro' );
