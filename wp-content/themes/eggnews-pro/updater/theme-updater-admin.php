					...
					<tbody>
						...
						<tr valign="top">
							...
							<td>
								...
								if ( 'valid' == $status ) { ?>
									<input type="submit" class="button-secondary" name="<?php echo $this->theme_slug; ?>_license_deactivate" value="<?php esc_attr_e( $strings['deactivate-license'], 'eggnews-pro' ); ?>"/>
								<?php } else { ?>
									<input type="submit" class="button-secondary" name="<?php echo $this->theme_slug; ?>_license_activate" value="<?php esc_attr_e( $strings['activate-license'], 'eggnews-pro' ); ?>"/>
								<?php }
								?>
							</td>
						</tr>
	 ...
	 */
	 function get_api_response( $api_params ) {
		...
		if ( is_wp_error( $response ) ) {
			wp_die( $response->get_error_message(), __( 'Error', 'eggnews-pro' ) . $response->get_error_code() );
		}
	...
	function activate_license() {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = __( 'An error occurred, please try again.', 'eggnews-pro' );
			}
			...
			if ( false === $license_data->success ) {
				...
				switch( $license_data->error ) {
					...
					case 'expired' :
						...
						$message = sprintf(
							__( 'Your license key expired on %s.', 'eggnews-pro'),
					...
					case 'revoked' :
						...
						$message = __( 'Your license key has been disabled.', 'eggnews-pro' );
						break;

					case 'missing' :

						$message = __( 'Invalid license.', 'eggnews-pro' );
						break;

					case 'invalid' :
					case 'site_inactive' :

						$message = __( 'Your license is not active for this URL.', 'eggnews-pro' );
						break;

					case 'item_name_mismatch' :

						$message = sprintf( __( 'This appears to be an invalid license key for %s.', 'eggnews-pro' ), $args['name'] );
						break;

					case 'no_activations_left':

						$message = __( 'Your license key has reached its activation limit.', 'eggnews-pro' );
						break;
